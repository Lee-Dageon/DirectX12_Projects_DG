...
//	CGameObject *pGameObject = CGameObject::LoadGeometryFromFile(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, "Model/Apache.bin");
	...
	pGameObject->Rotate(15.0f, 180.0f, 0.0f); // Y축 기준으로 180도 회전 추가
...
//	m_pTailRotorFrame = FindFrame("black_m_7");
	...
	m_pTurretFrame = FindFrame("TURRET"); // 탱크 포탑 찾기
}
...
void CAirplanePlayer::Animate(float fTimeElapsed, XMFLOAT4X4 *pxmf4x4Parent)
{
	...
	if (m_pTurretFrame)
	{
		XMMATRIX xmmtxRotate = XMMatrixRotationY(XMConvertToRadians(45.0f) * fTimeElapsed); // 초당 45도 회전
...
CCamera *CAirplanePlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	switch (nNewCameraMode)
	{
		...
		case SPACESHIP_CAMERA:
			...
			break;
		case THIRD_PERSON_CAMERA:
			SetFriction(300.0f); // 마찰력을 크게 증가해서 키를 떼면 바로 멈추도록
			SetGravity(XMFLOAT3(0.0f, 0.0f, 0.0f));
			SetMaxVelocityXZ(150.0f); // 최대속도를 엄청 크게 증가!
